...
public class Validators {
  ...
  public String validacaoCnpj(String cnpj) {
    ...
    if (cnpj.length() != 14) {
      throw new PecsException("O CNPJ deve conter 14 digitos númericos", HttpStatus.BAD_REQUEST);
    }
    ...
    if (ObjectUtils.notEqual(cnpj, cnpj.substring(0, 12) + digitoUm + digitoDois)) {
      throw new PecsException("O CNPJ está inválido", HttpStatus.BAD_REQUEST);
    }
  ...
  public String validacaoCpf(String cpf) {
    ...
    if (cpf.length() != 11) {
      throw new PecsException("O CPF deve conter 11 digitos númericos", HttpStatus.BAD_REQUEST);
    }
    ...
    if (ObjectUtils.notEqual(cpf, cpf.substring(0, 9) + digitoUm + digitoDois)) {
      throw new PecsException("O CPF está inválido", HttpStatus.BAD_REQUEST);
    }
