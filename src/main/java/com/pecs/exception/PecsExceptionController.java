...
public class PecsExceptionController {
        ...
        public ErrorHandlingFields handleConstraintViolationExcpetion(ConstraintViolationException exception,
                ...
                exception.getConstraintViolations().forEach(error -> errors.put(
                                Objects.requireNonNull(
                                                StreamSupport.stream(error.getPropertyPath().spliterator(), false)
        ...
        public ResponseEntity<ErrorHandling> handleUserException(BadRequestException exception,
                ...
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                                .body(
                                                new ErrorHandling(LocalDateTime.now(), HttpStatus.BAD_REQUEST.value(),
        ...
        public ResponseEntity<ErrorHandling> handleUserException(HttpServletRequest request) {
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                                .body(
                                                new ErrorHandling(LocalDateTime.now(), HttpStatus.UNAUTHORIZED.value(),
                                                                request.getServletPath(), request.getMethod(),
        ...
        public ErrorHandling handleServletRequestParameterException(MissingServletRequestParameterException exception,
                ...
                return new ErrorHandling(
                                LocalDateTime.now(), HttpStatus.BAD_REQUEST.value(), request.getServletPath(),
                                request.getMethod(), exception.getParameterName().concat(" é " +
                                                "obrigatório na requisição"));
        }

        @ExceptionHandler(PecsException.class)
        public ResponseEntity<ErrorHandling> handleUserException(PecsException exception, HttpServletRequest request) {
                return ResponseEntity.status(exception.getHttpStatus())
                                .body(
                                                new ErrorHandling(LocalDateTime.now(),
